...
void Obstacle::SetObstacle()
{
   //불화분 및 링 설정
	...
	for (int i = 0; i < OB_MAX; i++)
	{
		...
		m_arrFire[i].m_Rect1 = { m_arrFire[i].m_Point.x, m_arrFire[i].m_Point.y, m_arrFire[i].m_Point.x + IMAGE_SIZE, m_arrFire[i].m_Point.y + IMAGE_SIZE };
		m_arrRing[i].m_iIndex = IMAGE_ENEMY1;
		m_arrRing[i].m_Size = { IMAGE_SIZE, 250 };
		m_arrRing[i].m_Point = { RPoint + WIN_SIZE, WIN_SIZE / 2 };
		m_arrRing[i].m_Rect1 = { m_arrRing[i].m_Point.x, m_arrRing[i].m_Point.y, m_arrRing[i].m_Point.x + IMAGE_SIZE, m_arrRing[i].m_Point.y + IMAGE_SIZE };
		//중복또는 불가능 위치 생성 제어
		if (i != 0)
		{
			for (int j = 0; j < i; j++)
	...
	m_Goal.m_Rect1 = { WIN_SIZE * 10, CHAR_PT_Y + 50, WIN_SIZE * 10 + IMAGE_SIZE, CHAR_PT_Y + IMAGE_SIZE + 50 };
