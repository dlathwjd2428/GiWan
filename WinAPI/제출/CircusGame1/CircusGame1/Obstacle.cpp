...
void Obstacle::SetObstacle()
{
	...
	for (int i = 0; i < FIRE_MAX; i++)
	{
		...
		for (int j = 0; j < i; j++)
		{
			if ((Rpoint < POINT_X + 100) ||((Rpoint >= m_arrFire[j].m_Point.x - 300) && (Rpoint <= m_arrFire[j].m_Point.x + 500)))
			{
				Check = false;
				i--;
				break;
	...
	m_Goal.m_Point = { WIN_X * 10, POINT_Y };
...
void Obstacle::CreateRing()
{
	...
	if (RNum < 5 && m_vRing.size() <= 3)
	{
		...
		{
			if (WIN_X + 200 - m_vRing[i].m_Point.x < 300)
				return;
		}
