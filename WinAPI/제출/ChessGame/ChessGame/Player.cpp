...
void Player::PawnCheck(HWND hWnd)
{
	POINT tmpPt;
	int tmpImage;
	int Add = 3;
	...
	for (int i = 0; i < PIECE_MAX; i++)
	{
		if ((m_parrPiece[i]->GetImageIndex() == IMAGE_BLACK_PAWN && m_parrPiece[i]->GetRect().top <= 0)
			|| (m_parrPiece[i]->GetImageIndex() == IMAGE_WHITE_PAWN && m_parrPiece[i]->GetRect().bottom >= IMAGE_SIZE * 8))
		{
			tmpPt = { m_parrPiece[i]->GetRect().left, m_parrPiece[i]->GetRect().top };
			tmpImage = ChangePawn(hWnd, i) + Add;
			m_parrPiece[i]->SetInfo(tmpImage, tmpPt);
			break;
		}
	}
}

int Player::ChangePawn(HWND hWnd, int Index)
{
	...
	for (int i = 0; i < 5; i++)
	{
		...
		case 0:
			...
			{
				delete m_parrPiece[Index];
				m_parrPiece[Index] = new Pawn;
				return 0;
			}
			break;
		case 1:
			...
			{
				delete m_parrPiece[Index];
				m_parrPiece[Index] = new Knight;
				return 1;
			}
			break;
		case 2:
			if (MessageBox(hWnd, L"비숍으로 바꾸시겠습니까?", L"룩바꾸기", MB_OKCANCEL) == IDOK)
			{
				delete m_parrPiece[Index];
				m_parrPiece[Index] = new Bishop;
				return 2;
			}
			break;
		case 3:
			if (MessageBox(hWnd, L"룩으로 바꾸시겠습니까?", L"비숍바꾸기", MB_OKCANCEL) == IDOK)
			{
				delete m_parrPiece[Index];
				m_parrPiece[Index] = new Rock;
				return 3;
			}
			break;
		case 4:
