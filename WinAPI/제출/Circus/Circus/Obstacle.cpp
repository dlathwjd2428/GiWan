...
void Obstacle::SetObstacle()
{
   //불화분 설정
	...
	for (int i = 0; i < RING_MAX; i++)
	{
		...
		if (i == 0)
			m_arrRing[i].m_iIndex = IMAGE_ENEMY3;
		else
	...
	m_Goal.m_Rect1 = { m_Goal.m_Point.x , m_Goal.m_Point.y - 50, m_Goal.m_Point.x + m_Goal.m_Size.cx, m_Goal.m_Point.y + m_Goal.m_Size.cy };
...
void Obstacle::Draw(HDC hdc)
{
	...
	for (int i = 0; i < RING_MAX; i++)
	{
		...
		else if (m_arrRing[i].m_iIndex == IMAGE_ENEMY2)
			m_arrRing[i].m_iIndex = IMAGE_ENEMY1;
		else
			m_arrRing[i].m_iIndex = IMAGE_ENEMY3;
...
void Obstacle::JumpMove(bool MoveState)
{
	...
	if (MoveState == true)
	{
		...
		m_iRingSpeed = NORMAL_SPEED;
	m_Goal.m_Point.x += Add;
	m_Goal.m_Rect1 = { m_Goal.m_Point.x , m_Goal.m_Point.y - 30, m_Goal.m_Point.x + m_Goal.m_Size.cx, m_Goal.m_Point.y + m_Goal.m_Size.cy };
	MoveFire(Add);
}
...
int Obstacle::CollideCheck(RECT CharRect)
{
	...
	for (int i = 0; i < RING_MAX; i++)
	{
		...
		{
			Score += RING_SCORE;
			if (m_arrRing[i].m_iIndex == IMAGE_ENEMY3)
			{
				Score += FIRE_SCORE;
				m_arrRing[i].m_iIndex = m_arrRing[i + 1].m_iIndex;
			}
